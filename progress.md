...
**Done — 2026-08-02 audit (net-of-cost reality check):**
- Regime **SHORT cap** (`SHORT_CAP_BY_REGIME`, mirror of long cap) + **per-run same-direction concentration cap** (`MAX_SAME_DIRECTION_PER_RUN=2`) in `enforce_setups`. Both 07-30 loss streaks were 3× correlated shorts fired in one run.
- **Transaction cost model** in `weekly_eval` (`trade_cost_rr`): fee+slippage+funding → R via each trade's `risk_pct`; `net_rr`/`cost_rr`/`risk_pct` stamped per eval; net columns + net-of-cost verdict in `head_to_head.md`.
- **Lever #1 — longs require validated signal backing** (`LONG_REQUIRE_SIGNAL_BACKING=True`). Discretionary longs were the ENTIRE net loss (185t, 29% WR, −0.25R net). Signal-backed longs still pass in all regimes; off-by-default `ALLOW_DISCRETIONARY_LONGS_IN_RISK_ON` escape kept for future A/B.
- **KEY FINDING — no source survives costs yet.** Whole book gross −0.10R → **net −0.17R**. `signal_backed` +0.049R gross → **−0.009R net** (breakeven). Short-only book ≈ **−0.03R net**. The gross "edge" was mostly transaction cost. This reframes everything below: stop optimizing gross R; the only metric that matters now is **net-of-cost expectancy**.

**Live validated signals (2026-09-10 monthly re-validation):** EXECUTE = `trend_pullback_short` (**4h only** — holds thin: top combo +0.037 gross/100% robust, net-negative after cost; kept until the ~Oct 8 fork-A checkpoint, then the set gets restructured in one pass), `failed_breakout_short` (**4h only** — +0.096/N=63/67%), `liquidity_sweep_long` (**BOTH TF again — the only EXECUTE long, rarely fires live**). WATCH = `rsi_bounce_long` (4h) + **NEW 2026-09-10:** `range_reversion_short` (★ +0.250/N=30/100%) and `range_reversion_long` (★ +0.196/N=23/100%, long-side coverage) — both reject→strong flips, so they need a 2nd consecutive monthly pass before EXECUTE. **REMOVED 2026-09-10:** `rsi_rejection_short` 1h (failed re-val; its 4h variant flipped ★ STRONG → candidate pending 2nd pass, pairs with the CVD fork). `macd_momentum_*` stay out (repeat flip-floppers). Full detail in CLAUDE.md glossary → "Validated signals". Mechanical is LOW-FREQUENCY (0-1 setups/scan) and short-dominant (a run with only unbacked longs delivers nothing — by design).
...
5. ~~**Phase 4 — flip to mechanical-primary.**~~ **DONE 2026-08-26 (commit `8488cf7`) — but the original gate was BYPASSED.** The plan was to flip ONLY after head_to_head showed mechanical NET ≥ Claude NET over ≥20 trades each + concentration guard clear. That gate was **never met** (mechanical still net-negative, low-frequency, concentration guard not clear) — the user chose to flip anyway on a *philosophical* basis: the edge must live in our Python and we should stop paying for a shadow that isn't a decision-maker. So `PRIMARY_SOURCE="mechanical"`, the scan Claude call is gated off, and head-to-head is retired. **This does NOT mean the edge is proven** — it isn't; we're still SEARCHING per the north star. The flip just locks in the architecture + stops the spend. (`version_markers.json` cutover was NOT written since this isn't a selection-logic change to A/B — delivery simply switched from the already-shadowed mechanical source to itself.)
6. **Phase 5 — owned ML meta-filter.** scikit-learn take/skip model on `logs/evaluations/*` to replace Claude's discretion. Only after enough eval data (avoid overfitting).

_[NOW] Not data-gated — do when you want:_
7. **Signal expansion / 5th-6th signal hunt** — monthly `backtest`; promote out-of-sample winners into `_check_validated_signals` + `mechanical_setups`. **HELD candidate:** `failed_breakout_short_stacked` (Sep 10: now 100% robust on 4h, still 1h-overfit; stays held — largely duplicates `failed_breakout_short`). **Sep 10 additions routed through WATCH** (2nd-consecutive-pass rule for reject→strong flips): `range_reversion_short`/`_long` 4h; `rsi_rejection_short` 4h is the other pending candidate (★ STRONG Sep 10 after being dropped Jul 29 — also the signal the CVD slope-confirm fork pairs with). A strong long signal remains the highest-value add.

### SIGNAL-QUALITY ROADMAP (step-back review 2026-08-09) — the path past breakeven

**Diagnosis:** the system is *improving by subtraction* (PRE-gates 31% WR / −0.192R net → POST-gates 44% / −0.135R, n=82) but subtraction asymptotes at ~breakeven and can never reach profit. Root cause: **we compute lagging indicators (RSI/EMA/ADX/MACD) but do not model market STRUCTURE or LIQUIDITY** — the two things that determine where price actually reacts. Proof: **64% of stop-losses (108/168) never moved 0.5R our way** — entries are wrong *at entry*, not mismanaged. The next dollar of edge must come from a more *predictive* read, not another gate. **Method for every item below: build as a feature/signal → validate out-of-sample in `historical_backtester` (train/test + walk-forward) → promote ONLY if it survives.** Same discipline that got us from −0.23R to −0.14R, now aimed at predictiveness. All items are Python-owned (see CORE PRINCIPLE in CLAUDE.md — Claude never decides).

**META-CONCLUSION (2026-08-09, after 4 experiments this session) — price-derived mechanical signals do NOT beat costs on this universe.** Four independent tests all landed at −0.10 to −0.15R net: (1) market-structure filter (rejected, flips by TF), (2) downtrend-short filter (validated but marginal, underlying signal net-neg OOS), (3) S/R distance (real *feature* but bolted on a dead trigger = no fix), (4) NEW level-reaction reversal signal (−0.11 to −0.13R, net-positive in only 2/15 & 6/15 symbols, stable-NEGATIVE across all train/test splits). The consistency is the signal: gross ≈ breakeven, the ~0.07R taker cost is the whole gap. This is the signature of a near-efficient market (liquid USDT perps, 1h/4h) where simple price-pattern edges are arbitraged and fees decide the outcome. **Implication: more price-signal engineering is very unlikely to produce profit. Do NOT keep building signal #5/#6 by tuning price patterns.** The only forks with real upside: (A) a DIFFERENT information class — liquidation / order-flow data (the crypto-specific edge we keep naming but have NEVER tested; genuinely different info, not another price transform); (B) attack the COST — maker/limit entries (zero/neg fee) to stop fees eating a breakeven-gross book (honestly still not real edge, just less bleed); (C) accept the system is an analyst/research tool, not a money engine. **RESOLVED 2026-08-09: user chose (A) liquidation data — see fork-A item #3 below (CI collector live, backtest harness ready, data-gated ~2-4 weeks).** **UPDATE 2026-08-17:** the order-flow half of fork (A) has now been TESTED (was "never tested" above) — on FREE Binance deep history via `cvd_backtest.py`. Divergence/absorption/flow-level all dead, BUT `cvd_slope`-confirm on `rsi_rejection_short` passed the full bar (+0.11..+0.13R net 4h, robust/monotonic/OOS-stable/cross-TF) — the FIRST non-dead lever. Binance is a proxy → Bybit `cvd_collector.py` deployed to confirm on-venue (~3-4 weeks). See roadmap item #4 above for full results.

_Ranked by ROI (do top-down):_
1. ~~**Market-structure filter (BOS / CHoCH)**~~ **— PROTOTYPED & REJECTED as a universal gate (2026-08-09).** Tested a no-lookahead swing classifier (order-3 pivots, HH-HL/LH-LL → up/down/range) partitioning `trend_pullback` trades by structure-agreement, 15 symbols, both TFs. **The effect FLIPS sign between 4h and 1h** → noise, not edge: 4h short does WORST in a confirmed downtrend (−0.297R net, n=114) and BEST in a range (+0.017R, n=62); 1h is the exact opposite. A filter that reverses by timeframe is not shippable.
   - **Follow-up train/test on the one 4h-short lead (2026-08-09) — VALIDATED BUT NOT SHIPPED.** Chronological 60/40 held-out split, 15 symbols: skipping `trend_pullback_short` in an established 4h downtrend improves TEST net −0.282R→−0.163R, downtrend bucket is consistently the worst (TEST −0.383R/n=52, TRAIN −0.225R), worse in 7/12 symbols. So the effect is REAL (survives held-out, unlike the universal filter). **Not shipped for two reasons:** (a) even filtered it's still net-NEGATIVE (−0.163R) — pure subtraction on a losing signal, and subtraction has hit its ceiling; (b) **stability sweep exposed the real problem — the primary short signal is bleeding on the most recent window:** TEST net by split = 50/50 keep +0.044 / 60/40 keep −0.163 / **70/30 keep −0.555 (all −0.548 — filter does nothing, everything loses).** The most-recent ~30% is a −0.55R bloodbath (lines up with the Aug live regression; likely a risk_on/rally regime where shorts get run over — partly mitigated by `SHORT_CAP_BY_REGIME`). **Takeaway: the filter question is secondary — `trend_pullback_short` 4h is net-negative out-of-sample on recent data. Re-check its standing in the next monthly `backtest`; the real path to a positive signal is the level/liquidity work below, not more subtraction.**
2. **Real S/R & liquidity level map. — PROTOTYPED 2026-08-09: feature VALIDATED (robust OOS), but it's a feature not a fix.** Built a no-lookahead swing-cluster level map (strength = touches within 0.5 ATR) + direction-aware distance-to-nearest-strong-level (resistance overhead for shorts, support below for longs), tagged all `trend_pullback` trades, 15 symbols. **Finding (4h): entries in OPEN SPACE (>1.5 ATR from any strong level) are reliably the worst bucket — long far −0.447R/n=16, short far −0.834R/n=22 — and it HOLDS out-of-sample** (60/40 chrono split: TRAIN far −0.551R, TEST far −0.877R; all split ratios far −0.68 to −0.88R). This is the roadmap's core premise CONFIRMED: distance-to-a-real-level carries genuine predictive signal (unlike the structure filter, which flipped by TF; 1h also contradicts here, so the *filter* is 4h-specific). **BUT: (a) skipping open-space only nudges TEST net −0.376R→−0.325R (far is ~10-15% of trades); (b) KEEP (has-level) is STILL net-negative (−0.325R) and even the best in-sample bucket (4h long AT support) is only +0.016R = breakeven.** So the feature's power is on the DOWNSIDE (reliably flags bad entries) but doesn't surface a strong positive subset — because the `trend_pullback` TRIGGER is the ceiling (net-negative OOS, same as the downtrend train/test). **Conclusion: stop bolting filters onto trend_pullback. Two productive moves instead — (i) CAPTURE distance-to-level (+ structure state) as a live FEATURE stamped on setups/evals (like `interest_score`) so it's measurable live and feeds the Phase 5 ML meta-filter; (ii) build a level-ANCHORED entry signal (enter because price is reacting at a tested level + confirmation), validated fresh — not a filter on the old trigger.** Volume-profile/POC + prior day-week HL + round numbers still to add to the level map.
3. **Liquidation levels (crypto-specific edge). — FORK A CHOSEN 2026-08-09; free OI proxy TESTED = NO robust signal; real liq data untested (needs $ or forward-collection).** Reality: true historical liquidation data is NOT free — Bybit's liq feed is a real-time WS stream (forward-only, no historical endpoint), Coinglass historical is PAID. So tested the closest FREE proxy: historical Open Interest (Bybit `get_open_interest`, paginated+cached). Tagged all 4h trend_pullback trades with 1d OI-delta + price-vs-OI at entry (n=392 with coverage). **In-sample there was a coherent story (rising OI = worse, −0.22R; "new shorts" price↓+OI↑ = −0.33R) but it REVERSES out-of-sample** — every promising bucket flips sign on the held-out 40% (e.g. OI-down bucket TRAIN +0.273 → TEST −0.231; price↑OI↑ TRAIN +0.068 → TEST −0.523). Coarse OI-delta does NOT robustly separate winners. **Scope: this rules out aggregate OI-delta as a simple feature; it does NOT test the real liquidation-CLUSTER hypothesis (distance to a liq magnet), which OI-delta can't capture.** To test the real thing: (a) FREE — forward-collect Bybit's liquidation WS stream now, revisit in ~2-4 weeks; or (b) PAID — Coinglass historical API for an immediate backtest. **STATUS 2026-08-09 (current — earlier journey states are superseded, don't re-read them as live):** user chose FREE forward-collection. Repo made PUBLIC (git-history secret audit CLEAN across 603 commits: no .env/.session ever tracked, no keys/tokens) → **CI collector LIVE**: `.github/workflows/liquidation_collector.yml` chains ~5.5h jobs every 6h and uploads each run's JSONL as a 90-day **artifact** (NOT commits → no git bloat), reusing the `WG_CONF` VPN. The Mac launchd stopgap (`com.user.liqcollector.plist`) is STOPPED. `liquidation_collector.py` = data-collector only, zero trading decisions (CORE PRINCIPLE). Accepted limits: cron-jitter gaps + ToS-gray (persistent daemon on Actions); fallback if throttled = Oracle free-tier VM. **Backtest harness READY:** `liq_cluster_backtest.py` (written + smoke-tested) builds side-aware liq-cluster magnets from a trailing window (no-lookahead), tags trend_pullback trades by distance-to-nearest-DIRECTION-ALIGNED cluster (long=sell-cluster below=longs flushed / short=buy-cluster above=shorts squeezed), chrono train/test. **→ NEXT ACTION lives as the top Short-Term backlog item: run it once ~2-4 weeks of artifacts exist; promote distance-to-liq-cluster into `mechanical_setups.py` ONLY if NEAR-aligned TEST net beats ALL out-of-sample and is ≥~0, else it's the 7th empty lever → fork C.** Prior free OI-delta proxy already tested (2026-08-09) = no robust OOS signal (rules out coarse OI as a feature; does NOT test the real liq-cluster magnet). `liquidity_sweep_long` remains a crude price-only proxy until the real test runs.
   - **FIRST PASS RUN 2026-09-10 (31.3 days of artifacts, 22,014 unique events, 19 symbols — TONUSDT delisted) — INCONCLUSIVE: promotion bar NOT MET, but the strongest relative lead of any fork so far; extended ONE cycle on data-quality grounds.** Data quality was the limiter: hour-level coverage only **44%** (~14 effective days) — pybit's WS dies QUIETLY after VPN blips (zombie collector; the local daemon sat 13h silent with VPN up) and 3/14 recent CI runs died at VPN bring-up. Results (net-of-cost, trend_pullback trades, fixed 0.072R cost): **4h too thin to split** (66 tagged, near=13 → the harness's own "keep collecting" branch fired). **1h (n=346): the 0.5–1.5-ATR "NEAR an aligned cluster" ring is the best bucket** — +0.213R net full-sample vs book −0.079; holds in BOTH hi-coverage (+0.232) and lo-coverage (+0.198) windows so it is NOT a gap artifact; not symbol-concentrated; **near-ring LONGS +0.396R net / 62% WR (n=26) — the first non-dead LONG lead this universe has produced**. BUT the pre-registered bar (NEAR TEST net ≥ ~0 AND clearly > ALL) **failed** on the harness split: NEAR TRAIN +0.051 → TEST **−0.066** (vs ALL TEST −0.144: relative gap +0.078R positive, absolute negative), and the split is UNSTABLE at this n (a global time-rank split gives NEAR TEST **+0.558**; by month Aug −0.02 vs Sep +0.60) = underpowered, not proven. The **AT (≤0.5A) ring is reliably bad and largely a low-coverage artifact** (hi-cov +0.086 vs lo-cov −0.361) — if this ever promotes, it's a RING feature (avoid ≤0.5A, favor 0.5–1.5A), 1h-flavored. **DECISION: strict reading of the pre-registered rule = fork C. Recommended + taken instead: one extension — the rule assumed 2-4 weeks of REAL data and we got ~14 effective days at 44% duty. Shipped the fix (`LIQ_STALL_SECONDS` watchdog in `liquidation_collector.py` rebuilds the WS after 45 min of silence; local daemon restarted on the new code) → RE-RUN ~2026-10-08 with ~2× effective data + the same slices (`liq_cluster_deep_dive.py`, new). If the near-ring still fails the bar then, fork C for real — no second extension.** Merged dataset: `logs/liquidations/ci/` (gitignored; artifacts also expire ~Nov, so this local copy is now the canonical store — re-download recipe in the 2026-09-10 changelog entry).
4. **Order flow: CVD + OI-delta. — TESTED 2026-08-17 on FREE Binance deep history; ONE robust flow feature found (`cvd_slope`), the FIRST non-dead lever. Now Phase-2 gated (forward-collect Bybit CVD to confirm on-venue).** Bybit has no free historical trade/CVD feed (recent-trade REST caps ~1000 trades), so tested the hypothesis on Binance USD-M monthly kline dumps (`data.binance.vision`, reachable without VPN; carry `taker_buy_base_volume` = per-bar CVD) via new `cvd_backtest.py` (research-only, zero trading decisions). Universe: the 15-symbol validation set, 4h/24mo + 1h/12mo, net-of-cost, chrono train/test + parameter sweeps. **Results:** (a) **CVD divergence standalone = DEAD** (net −0.04R, gross≈breakeven eaten by fees, long flips OOS). (b) **Absorption reversal standalone = DEAD** (every z-threshold net-negative, TRAIN≈0→TEST −0.15/−0.27, non-monotonic). (c) **per-bar taker-share *level* filter (`flow_ratio`) = REJECTED** — looked +0.09R at (k6,thr2%) but NOT robust: below-baseline at k4, negative at k9 (classic one-lucky-param overfit, same failure as OI-delta). (d) **★ CVD-SLOPE (flow-momentum) confirmation filter on `rsi_rejection_short` = PASSES the full bar** — confirm bucket net vs baseline +0.064→**+0.11..+0.13R** (4h), lifts 1h −0.073→~breakeven; **robust + MONOTONIC** across slope-thr 0.05→0.40 (edge *strengthens* with extremity), **OOS-stable** (TEST +0.24..+0.26 vs +0.14 baseline), **cross-TF consistent** (4h & 1h same direction), large N (42–64% of trades), clean confirm>baseline>contra ordering (contra reliably worst). Modest magnitude, **signal-SPECIFIC** (did nothing for `trend_pullback_short`). **Caveats before ship:** Binance-proxy not Bybit; per-symbol concentration unchecked; `rsi_rejection_short` is live-validated 1h-only. **→ Phase 2 (the justified next step): forward-collect real Bybit per-bar taker CVD (same pattern as the liq collector) → in ~3-4 weeks confirm the slope-confirm effect ON-VENUE → only then promote a slope-confirmed `rsi_rejection_short` variant into `mechanical_setups.py`.** OI-delta half already tested 2026-08-09 = no robust signal.
5. **Owned ML meta-filter (Phase 5) — AFTER 1-4.** Take/skip P(win) classifier on eval history. GIGO on today's thin features — only powerful once items 1-4 supply real inputs (dist-to-level, dist-to-liq, structure state, CVD). Tier 1 is a prerequisite, not a parallel track.

_De-prioritized (honest ROI call, 2026-08-09):_
- **Chart screenshots → Claude vision:** seductive but low ROI + violates our discipline — vision finds *textbook* shapes (widely known, rarely edgy) and is **nearly impossible to backtest cheaply** (render thousands of charts + pay per call), so you can't prove edge before shipping. Also slow/costly/token-heavy and would make Claude core (see CORE PRINCIPLE). Narrow OK-use: render OUR computed levels onto a chart as a Claude **veto/sanity** layer over a mechanical setup — a shadow experiment only, after items 1-3.
- **Digitize TA books into `knowledge/`:** lowest ROI — Claude already knows all standard TA from training; textbook patterns are common knowledge and mostly unquantified, so this adds ~zero *predictive* edge. If a book has a quantified/backtested rule, implement it in Python + validate — don't feed the prose.

**Net-of-cost slice cut (2026-08-09, 334 evals) + what shipped.** Re-ran the "which subset survives costs" cut on the fuller book. **NO slice with real n is net-positive** (only ★NET+ buckets are n=2–3 noise). Firm structural facts: longs −0.242R/29%/n=186 vs shorts −0.097R/41%/n=148 (lever #1 already gates longs); **4/4 confluence is the WORST real bucket, net −0.305R/23%/n=71** vs 3/4 −0.095R/n=237; median MFE 0.77R so T2@1.5R hits only 13%; **50% stop out and 64% of those (108/168) never reached 0.5R MFE** (wrong-entry, not targeting); 284/334 trades are `trend_pullback` (one signal, not a portfolio). **Shipped:** escalated 4/4 from demote-only → **outright refusal** (`config.REFUSE_4OF4_CONFLUENCE=True` + violation in `main.py::setup_violations`, both directions, reversible). **Strategic conclusion: no net edge exists in the current signal family and no parameter fixes it — it's a signal-QUALITY problem. Upside only via a genuinely different signal source or the Phase 5 ML meta-filter (skip the 108 dead-on-arrival trades). Stop tuning knobs.** See [[project-net-of-cost-audit]].
...
- **Point 2 done — wider-universe test: FAILED the holdout; live universe NOT widened.** `range_reversion_short`+vol gate on 33 symbols never used to find the gate (top-60 by turnover minus the original 29; reports `unified_20260923_rrs_holdout33.md` / `_rrs_all61.md`): holdout all-period n=120 57.5% / −0.02R, TEST (last 40%) n=48 **50% / −0.21R**; crypto-only holdout TEST 45% / −0.28R. Combined 61 symbols: n=216 64.8% / +0.09R, TEST n=87 57.5% / −0.11R. **Month profile is the real story — monotonic decay on BOTH universes:** 61-symbol book Apr 82% / +0.56R → May 69% → Jun 66% → Jul 57% → Aug 65% → **Sep 50% / −0.23R**. The original 29-symbol 75% was carried by April–June; the rule has run ≈50–57% since July everywhere. **Honest prior for the pre-registered forward test is therefore 50–60%, not 75%.** The test still runs exactly as registered (it is cheap and the only clean answer), but the bar is now expected to FAIL; plan the next product accordingly rather than waiting idle. 1h variant not tested (the 4h failed its holdout; no reason to expand).
- **CI robustness (same night):** first dispatched run died on a Bybit `ReadTimeout`; second hit the 20-min job timeout because the VPN route to `api.bybit.com` was dropping kline calls one by one while `api.bytick.com` (used by the pulse) answered. Third run "succeeded" in 18 min with 17/30 symbols missing: `api.bytick.com` answers **403 "ip is from the usa"** on US-located runners, so the failover ping-ponged and the abort was swallowed at the symbol level. **Final design (commit `b5127ed`, 4th dispatched run: 2 min 13 s, 30/30 symbols):** every fetcher call goes through `BybitFetcher._api` — a NETWORK failure flips to the other live host and counts toward a budget of 4 *consecutive* failures (reset on any success); an API REJECTION (`FailedRequestError`, the 403) marks that host dead for the run; `BybitHostsDown` propagates out of the snapshot so `main.py` retries the whole fetch 3× (start host bybit → bytick → bybit). `momentum_pulse.fetch_tickers` tries bytick then bybit for the same 403 reason (it had been silently "aborting pulse" on US runners — exit 0, no hot-list refresh). **The WireGuard endpoint is the real dependency:** if `collector_health.yml` alerts or scans keep timing out, rotate `WG_CONF`.
- **State after tonight / what to do next.** (a) Nothing to do on the forward test but run `eval-scan` weekly and read `head_to_head.md` § "Eval engine v2 era" (velocity line + per-signal promotion status). (b) Check `gh run list --workflow=momentum_pulse.yml --limit 30` in a week: the goal is a run within 90 min after EVERY 4h close; if closes are still missed, add retry crons, do not widen the freshness window. (c) Because the rule's monthly profile decayed to ~50% by September on both universes, expect the pre-registered test to land in the <60% branch; use the waiting time to define the next product's success metric (weekly cross-sectional momentum is the only remaining direction with evidence behind it) rather than another 4h price-pattern idea. (d) Oct 8 liq-cluster re-run and the CVD Bybit-bucket loader stay on the list, but only as *filters tested on this rule in the shadow lane*, never as live changes during the test. (e) Risk engine (Phase B prerequisite, pure Python, wired to nothing) is optional filler work.

### 2026-09-23 (night) — Maker-fee fill model tested → adopted as the trade model (`ENTRY_MODEL="limit_open"`)

**Built.** `trade_sim.simulate_limit` (post-only limit at the next 15m open, improved by `offset×ATR`; fills only if price trades THROUGH it within `wait` bars — a touch is not a fill; simulated from the fill bar with the same engine) + per-leg fee model `cost_rr_legs` ("maker_entry": maker on entry and take-profits, taker+slippage on stops/BE/trail/expiry; the legacy "taker" path is byte-identical). `config.MAKER_FEE_PCT=0.0002`. Harness section (f) compares, per rule, market baseline vs 9 limit variants (offset {0, 0.15, 0.3} × wait {2, 4, 8}), and re-costs the SAME filled subset at taker so fee effect and selection effect are separated. Test added (`test_limit_entry_fill_and_fees`).

**Result (30 symbols, 180 d).** Limit at the open (offset 0) fills 96–99% of signals within 30 min and improves net by +0.03..+0.08R on EVERY rule with hit rate unchanged (pure fee effect): liquidity_sweep_long 1h −0.18 → −0.10; trend_pullback_short −0.07 → −0.03; failed_breakout_short −0.13 → −0.10; rsi_bounce_long −0.17 → −0.16; range_reversion_short +0.26 → **+0.33R at 79% (n=58)**; range_reversion_long −0.05 → +0.02. Offsets (0.15–0.3 ATR) raise hit rate a few points via selection but cut fills to 20–70% — not adopted. It does NOT flip the losing rules; it widens the margin on the one that works.

**Adopted.** `ENTRY_MODEL="limit_open"`, `LIMIT_WAIT_BARS=2`, `LIMIT_OFFSET_ATR=0.0` — used identically by the evaluator (`weekly_eval.evaluate_setup` → `simulate_limit`; unfilled → status `not_filled`, excluded like `not_triggered`; rows carry `entry_model`/`fee_model`/`fill_bar`), the harness default (`sim_trade`; `--entry market|limit_open` override; unfilled counted in the header) and the brief ("Entry: post-only LIMIT at ~price, cancel if not filled within 30 min — do NOT chase"). The 2026-09-23 report was regenerated under the new default.
...
- **Outage found during a "do we have enough data?" check:** the liq collector had **zero clean CI successes since Sep 10** (24 cancelled + 5 failed runs; only 0.1 MB of artifacts in 7 days vs ~1.7 MB the prior month), CVD partially degraded, and even 4 scan runs died — an entire week of fork-A collection mostly lost with no signal to the user. Root cause, two layers: (1) the WireGuard endpoint went flaky ~Sep 15 (tunnel handshakes but Bybit WS gets ping/pong timeouts or can't connect); (2) the tunnel was **full-tunnel (0.0.0.0/0)**, so when the endpoint degraded the RUNNER ITSELF lost contact with GitHub → GitHub killed the job ("The operation was canceled", uniform ~8-min cancels across all three workflows).
- **Fix 1 — split-tunnel VPN** (`.github/actions/bybit-vpn` composite action, shared by all 3 workflows): resolve Bybit's CloudFront IPs on the clean runner network, pin them in `/etc/hosts` (so mid-run socket rebuilds reuse routed IPs), and set `AllowedIPs` to ONLY those /32s. GitHub/pip/Telegram traffic stays direct → a dead VPN endpoint can no longer take down the runner. Telegram from US runners needs no VPN (the ISP block is local-Indonesia only), so alerts survive VPN outages too. Plus a **health gate**: Bybit must answer `GET /v5/market/time` through the tunnel; 5 rebuild attempts, then fail fast with a clear error.
- **Fix 2 — retry across the window** (both collectors): a WS death (`sys.exit(1)`) previously forfeited the whole 6h window after ~2 min. Now the step rebuilds the tunnel and relaunches the collector with the REMAINING second-budget until the 5.5h window is spent — the endpoint flaps, so later attempts recover mid-window. Clean self-exit → success; budget spent without one → failure (honest signal for monitoring).
- **Fix 3 — `collector_health.yml`**: daily 06:45 UTC no-VPN watchdog; reads the last 24h of run conclusions via the GitHub API and Telegram-alerts when any workflow is under 50% success. Tested live: correctly reported liq 0/4 / cvd 0/3 and delivered the alert. The "watchdog for the watchers" the Sep 10 stall-watchdog era was missing.
- **Fork-A schedule impact:** ~7 of the 28 extension days were lost at ~pre-watchdog coverage. If the endpoint itself stays unhealthy even through the split tunnel, the remaining fix is rotating `WG_CONF` to a fresh server config — the health alert now makes that visible within a day instead of a week. Re-check data volume before the ~Oct 8 re-run; extend the window if coverage stayed poor (the extension was granted on data-quality grounds; a collection outage is the same grounds).
- (Same session: the local `venv/` had been deleted — rebuilt with Python 3.14 + `requirements.txt`; all local aliases (`scan`/`pulse`/`eval-scan`/`backtest`/`trade`) share it and were broken.)

### 2026-09-10 (3) — Monthly signal re-validation (first since Jul 29) + live-set changes

- Ran the full `backtest` pipeline on fresh data (15 symbols, 4h + 1h, 18 formulas). **Live set verdicts:** `trend_pullback_short` 4h HOLDS by a thread (top combo +0.037 gross/100% robust, #2-5 marginal/overfit — matches its live 43-trade record of +0.008 gross ≈ −0.05 net); `failed_breakout_short` 4h HOLDS (+0.096/N=63/67%); `liquidity_sweep_long` confirmed BOTH TFs again (1h +0.098/100%). **`rsi_rejection_short` FAILED on 1h** (every top combo test-negative on fresh data) while its 4h variant flipped to ★ STRONG (+0.283/N=82/100%) → **1h detection REMOVED** per the drift rule; only 2 live trades existed, so the cost is ~zero. The 4h variant is a promotion candidate pending a **2nd consecutive monthly pass** (new standing rule for reject→strong flip-flops) and pairs with the CVD slope-confirm fork.
- **Added `range_reversion_short` + `range_reversion_long` as WATCH-tier signals** (4h, validated params: short rsi>70/adx<25/range≥5%/prox 1%/stop high_20+0.7·ATR; long rsi<30/adx<18/range≥4%/prox 1.5%/stop low_20−0.3·ATR; target 1.5R, structural `stop_price`). Both ★ STRONG on Sep 10 (+0.250/N=30 and +0.196/N=23, 100% robust) but previously rejected → 2nd-pass rule applies. The long doubles as long-side coverage (the only EXECUTE long has fired 0 times live in ~7 weeks). `macd_momentum_*` passed again on one TF each but stay out (repeat flip-floppers). 6/16 formulas flipped status in 6 weeks — reinforces the 08-09 "price-pattern layer is regime-noisy" conclusion.
- **Watch-lane attribution fixed** (`weekly_eval.py`): eval results now carry `signal_name` and the promotion readout keys on it — previously all 74 watch trades collapsed into one "(unknown)" row, blinding the per-signal table the readout exists for. Legacy watch evals stay unlabeled; new ones split correctly.
- Verified: sliding-window smoke over 20 symbols → 0 `rsi_rejection_short` fires, both range signals fire at plausible rates with `tier=watch` + structural stops; all 6 `test_signal_levels` drift tests pass.
- **Anchor-demotion guard** (`mechanical_setups._build_one`, found while verifying "can this change worsen the book?"): a WATCH co-fire with higher gross expectancy could hijack a mixed same-direction group's anchor and demote an execute-worthy setup into the watch lane — latent since `rsi_bounce_long`, about to become live-relevant (`range_reversion_long` 0.20 co-fires with `liquidity_sweep_long` 0.04–0.08 at swept range bottoms → would have suppressed the only EXECUTE long). Now EXECUTE signals always outrank WATCH for anchoring; watch anchors only all-watch groups. Verified with a synthetic co-fire test (mixed→execute, watch-only→watch).
- **Expected live impact (audit against this at the next eval-scans):** EXECUTE book composition unchanged except the absence of `rsi_rejection_short` 1h (2 trades in 7 weeks, 1W/1L — counterfactual counted WR 51.1%→51.2%, i.e. nil; the formula is freshly test-negative so future absence is neutral-to-positive); WATCH lane gains two validated-formula candidates that outrank observation fallbacks; **measured win rate / net expectancy cannot regress by construction** (watch is excluded from every counter). This is hygiene + candidate incubation, NOT edge creation — the book stays net-negative until a predictive input lands.
- **Decision recorded:** `trend_pullback_short` 4h (net-negative but ~95% of execute flow) stays live until the ~Oct 8 fork-A checkpoint — the signal set gets restructured there in one pass, informed by the fork-A re-run.
...
- Downloaded all 112 `liquidations-*` CI artifacts + merged with the 2 local daemon files → 22,014 unique events / 31.3 days / 19 symbols → `logs/liquidations/ci/` (gitignored; now the canonical store since artifacts expire after 90 days). Re-download recipe: `gh api "repos/RenaldoLouis/Telegram_crypto/actions/artifacts?per_page=100" --paginate --jq '.artifacts[] | select(.expired==false and (.name|startswith("liquidations"))) | "\(.id) \(.name)"'` → `gh api "repos/.../actions/artifacts/<id>/zip"` per row → unzip each as `<artifact>_<name>.jsonl` into `logs/liquidations/ci/` (the loader dedups).
- Refreshed the 20-symbol kline cache (4h + 1h; the harness reads cache-first and it was stale from Jul 29/Aug 9 — a re-run MUST refresh or it silently tags zero trades). TONUSDT is delisted on Bybit (klines end Jun 15) — self-excludes.
- Ran `liq_cluster_backtest.py` on 4h + 1h, plus new `liq_cluster_deep_dive.py` (per-symbol / per-direction / per-month / coverage-sensitivity / alternate chrono split — keep slices identical for the re-run).
- **RESULT: promotion bar NOT met** (1h NEAR TEST −0.066 < 0 on the harness split) **but** the 0.5–1.5A near-ring beats the book OOS by +0.078R, survives coverage sensitivity, and near-ring LONGS are +0.396R net / 62% WR (n=26) — the first non-dead long lead. Underpowered: collector duty cycle was only 44% (~14 effective days). Full detail + decision in roadmap item #3.
- **Root cause fixed:** pybit WS dies quietly after VPN blips → zombie collectors. Added `LIQ_STALL_SECONDS` watchdog (default 2700s) to `liquidation_collector.py` — rebuilds the socket after 45 min of zero events across 20 perps; restarted the local launchd daemon on the new code (it had been silent 13h with VPN up; progress.md previously said it was stopped, but it had come back at login — kept deliberately now as gap-filler while CI is flaky, dedup handles the overlap). **Re-run scheduled ~2026-10-08; if the bar fails again → fork C, no second extension.**

### 2026-08-26 — v12.0 migration complete: mechanical-primary, Claude off the scan path
- **Flipped `PRIMARY_SOURCE` → `"mechanical"`** (commit `8488cf7`). Mechanical is now the only delivered scan source, everywhere (12 CI + 3 local runs).
- **Claude scan call gated off** — `main.py` wraps the shadow block in `if config.PRIMARY_SOURCE == "claude":`, so `analyzer.analyze()` is never invoked on the scan path (CI *or* local). **Zero scan Claude tokens.** Claude now only spends on `eval-scan` delta analysis.
- **Head-to-head shadow retired** — vestigial; the real bar is +0.05R net-of-cost, not "beat Claude". `generate_head_to_head` still runs but the `claude` lane stops accruing new trades (history kept).
- **Phase-4 flip gate bypassed deliberately** (user decision) — flip was philosophical (own the edge / stop paying for a non-decision-maker), NOT a proof mechanical beats Claude. **Edge remains UNPROVEN** — mechanical still net-negative, still SEARCHING per the north star. Fully reversible via `PRIMARY_SOURCE="claude"`.
- Docs synced: CLAUDE.md § Setup Source + CORE PRINCIPLE + Scheduled Runs, progress.md Current Status/roadmap/checklist.
...
- [ ] **★ ACTIVE NEXT ACTION — run the liquidation-cluster backtest (data-gated, ~2-4 weeks from 2026-08-09).** Once the CI `Liquidation Collector` has accumulated enough data: GitHub → Actions → download the `liquidations-*` artifacts, unzip all `liq_*.jsonl` into one dir → `python liq_cluster_backtest.py --data-dir <dir>` (also `--interval 60` for 1h). Promote distance-to-liq-cluster into `mechanical_setups.py` ONLY if NEAR-aligned TEST net beats ALL out-of-sample and is ≥~0; else it's the 7th empty lever → fork C (analyst-tool). Full context: fork-A item #3 in the roadmap above.
- [ ] **★ SECOND DATA-GATED ACTION — confirm the CVD `cvd_slope` edge on-venue (data-gated, ~3-4 weeks from 2026-08-17).** `cvd_collector.py` is live on CI (`.github/workflows/cvd_collector.yml`) accumulating per-minute Bybit taker buckets. Once ~3-4 weeks exist: download the `cvd-*` artifacts, concat `logs/cvd/*.jsonl`, rebuild per-bar CVD + cvd_slope, and re-test whether the slope-CONFIRM lift on `rsi_rejection_short` (+0.11..+0.13R net on Binance 4h — robust/monotonic/OOS-stable) HOLDS on real Bybit data. Also do the per-symbol concentration check (skipped so far). Promote a slope-confirmed `rsi_rejection_short` variant into `mechanical_setups.py` ONLY if it holds on-venue AND net ≥ ~0. Full context: roadmap item #4 above. (Same 90-day artifact expiry caveat as the liq artifacts.)
- [ ] **Check mechanical NET-of-cost expectancy** (recurring) — after each `eval-scan`, watch the **NET-of-cost** verdict for the `mechanical` lane in `logs/performance/head_to_head.md` (not counts, not gross R). Head-to-head vs Claude is now RETIRED (flip done 2026-08-26); the `claude` lane no longer accrues new trades, so read the mechanical row against the **+0.05R edge bar**, not against Claude. As of 2026-08-09: mechanical was net −0.098R — still net-NEGATIVE, edge UNPROVEN. Goal: drag mechanical net toward ≥+0.05R over ≥100 forward trades with both directions + >1 signal.
- [ ] **Monthly `backtest` re-validation** — re-run `backtest`, reconcile `_check_validated_signals` + `mechanical_setups.EXPECTANCY/SETUP_TYPE/SETUP_RULE` to fresh results, promote/demote signals (last full run 2026-07-21). This RE-VALIDATES existing signal formulas — it is NOT price-signal hunting, which the 2026-08-09 meta-conclusion proved a dead end (4 tests all net-negative; stop building new price-pattern signals). HELD candidates if ever revisited: `failed_breakout_short_stacked` (4h N=12/0% robust — fragile), `liquidity_sweep_short` (single-TF only).
- [ ] **Per-symbol / long-blacklist review** — revisit the long blacklist with fresh eval data (ETHUSDT already blacklisted; BTC/ETH historically weak long). Low priority.

### Medium Term (next 1-2 months)
- [ ] Enable Telegram signal group reading (add groups to config)
- [ ] CoinGlass data (PAID) — only a fallback if the FREE liq-cluster collection (fork A) proves out but needs deeper history than forward-collection gives. Note: OI-delta already tested (2026-08-09) = no robust signal; funding/OI are not a priority.
