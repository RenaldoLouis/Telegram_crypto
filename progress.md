...
**Live validated signals (Jul 29 2026 re-validation):** `rsi_rejection_short` (**1h only**), `trend_pullback_short` (**4h only**), `failed_breakout_short` (**4h only**), `liquidity_sweep_long` (**BOTH TF — the only long signal**). REMOVED as overfit: `macd_momentum_long` + `macd_momentum_short`. Full detail in CLAUDE.md glossary → "Validated signals". Mechanical is LOW-FREQUENCY (0-1 setups/scan) and, post-lever-1, short-dominant (a run with only unbacked longs now delivers nothing — by design).
