...
5. **Phase 4 — flip to mechanical-primary.** ONLY after head_to_head shows mechanical **NET** expectancy ≥ Claude NET, ≥20 evaluated trades EACH, **AND the concentration guard is clear** (mechanical book has BOTH directions + >1 signal live). Mechanical is currently short-only / 1-signal → NOT flip-ready regardless of expectancy. Set `PRIMARY_SOURCE="mechanical"` + write `version_markers.json` cutover. Reversible. **CAVEAT:** mechanical accumulates ~6×/day vs Claude 1×/day → asymmetric counts; judge on per-source net expectancy, treat "≥20 each" as a floor.
...
- [ ] **Check head-to-head data** (recurring) — after each `eval-scan`, watch the **NET-of-cost** verdict in `logs/performance/head_to_head.md` (not counts, not gross R). As of 2026-08-09: 334 evals — mechanical 39 (net −0.098R), Claude 295 (net −0.191R); both net-NEGATIVE so NOT flip-ready, and the concentration guard is not clear (mechanical is short-only / 1-signal). See the Phase 4 flip criteria above.
