...
1. ~~**Market-structure filter (BOS / CHoCH)**~~ **— PROTOTYPED & REJECTED as a universal gate (2026-08-09).** Tested a no-lookahead swing classifier (order-3 pivots, HH-HL/LH-LL → up/down/range) partitioning `trend_pullback` trades by structure-agreement, 15 symbols, both TFs. **The effect FLIPS sign between 4h and 1h** → noise, not edge: 4h short does WORST in a confirmed downtrend (−0.297R net, n=114) and BEST in a range (+0.017R, n=62); 1h is the exact opposite. A filter that reverses by timeframe is not shippable. **One TF-specific lead worth a targeted train/test:** 4h `trend_pullback_short` in an *established* downtrend = −0.30R/n=114 — the same "full alignment = late/exhausted" lesson as 4/4 confluence; candidate refinement to the 4h short signal ONLY (not a universal filter), needs a real train/test split (proto was in-sample). Lesson: "trade with the trend" is false for this pullback signal on 4h. Reorders the roadmap — level/liquidity items are now the top unproven bets.
