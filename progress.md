...
- [ ] **Next lever — hunt a 5th/6th validated signal** — the real multiplier for setups/run (only 4 live; scans yield ~1/run because coins aren't the bottleneck, signal count is). Use `historical_backtester.py` (`--optimize`/`--validate`/`--walkforward`) to search candidate patterns; promote only those passing out-of-sample train/test + robustness into `_check_validated_signals` + `mechanical_setups.EXPECTANCY/SETUP_TYPE/SETUP_RULE`. HELD candidates to re-check with more data: `failed_breakout_short_stacked`, `liquidity_sweep_*`. Own focused session; deferred 2026-07-28 pending more head-to-head data.
