...
3. **Liquidation levels (crypto-specific edge).** Bybit free liquidation stream first; Coinglass heatmap API if it proves out. Liq clusters are magnets/reversal zones — `liquidity_sweep_long` is already a crude proxy (and our only long-edge hint). Highest ceiling of the data items.
