...
3. **Per-signal T1 + direction review** (was gated at ~20 mechanical evals; we're at 23, but lever #1 just changed selection). Wait for post-lever-1 evals, then check per-signal net expectancy and optimal T1 distance.
4. **Phase 4 — flip to mechanical-primary.** ONLY after head_to_head shows mechanical **NET** expectancy ≥ Claude NET, ≥20 evaluated trades EACH, **AND the concentration guard is clear** (mechanical book has BOTH directions + >1 signal live). Mechanical is currently short-only / 1-signal → NOT flip-ready regardless of expectancy. Set `PRIMARY_SOURCE="mechanical"` + write `version_markers.json` cutover. Reversible. **CAVEAT:** mechanical accumulates ~6×/day vs Claude 1×/day → asymmetric counts; judge on per-source net expectancy, treat "≥20 each" as a floor.
5. **Phase 5 — owned ML meta-filter.** scikit-learn take/skip model on `logs/evaluations/*` to replace Claude's discretion. Only after enough eval data (avoid overfitting).

_[NOW] Not data-gated — do when you want:_
6. **Grade the 6 experimental delta insights** in `rule_registry.json` — all still `experimental` after weeks; the grading discipline lapsed. Confirm/expire each (this is the antibody against the rule-accumulation loop). Run via delta analysis or manual review.
