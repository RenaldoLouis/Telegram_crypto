...
_As of 2026-08-02. Single source of truth for where the mechanical-primary migration stands and what to do next. Full plan: `~/.claude/plans/ok-becauase-in-my-refactored-locket.md`._
...
3. **Liquidation levels (crypto-specific edge). — FORK A CHOSEN 2026-08-09; free OI proxy TESTED = NO robust signal; real liq data untested (needs $ or forward-collection).** Reality: true historical liquidation data is NOT free — Bybit's liq feed is a real-time WS stream (forward-only, no historical endpoint), Coinglass historical is PAID. So tested the closest FREE proxy: historical Open Interest (Bybit `get_open_interest`, paginated+cached). Tagged all 4h trend_pullback trades with 1d OI-delta + price-vs-OI at entry (n=392 with coverage). **In-sample there was a coherent story (rising OI = worse, −0.22R; "new shorts" price↓+OI↑ = −0.33R) but it REVERSES out-of-sample** — every promising bucket flips sign on the held-out 40% (e.g. OI-down bucket TRAIN +0.273 → TEST −0.231; price↑OI↑ TRAIN +0.068 → TEST −0.523). Coarse OI-delta does NOT robustly separate winners. **Scope: this rules out aggregate OI-delta as a simple feature; it does NOT test the real liquidation-CLUSTER hypothesis (distance to a liq magnet), which OI-delta can't capture.** To test the real thing: (a) FREE — forward-collect Bybit's liquidation WS stream now, revisit in ~2-4 weeks; or (b) PAID — Coinglass historical API for an immediate backtest. **DECISION: user chose (a) FREE forward-collection (2026-08-09).** `liquidation_collector.py` is LIVE as a launchd KeepAlive daemon (`com.user.liqcollector.plist`) — subscribes to Bybit `all_liquidation_stream` for ~20 liquid perps, appends raw prints to `logs/liquidations/liq_YYYY-MM.jsonl` (gitignored). Verified: captures real events on connect (data-collector only, zero trading decisions). Collects while Mac awake + VPN up (expect gaps). **UPDATE 2026-08-09: moving host to CI to remove Mac-dependency — user opted to make the repo PUBLIC (git-history secret audit run first = CLEAN across 603 commits: no .env/.session ever tracked, no keys/tokens). Added `.github/workflows/liquidation_collector.yml` — chained ~5.5h jobs every 6h, uploads each run's JSONL as a 90-day artifact (NOT commits → no git bloat), reuses `WG_CONF` VPN. Activates once the repo is public. Stop the Mac launchd daemon once CI is confirmed live (else double-collection). Known limits accepted: cron jitter gaps, ToS-gray (persistent daemon on Actions).** **Repo made public + CI collector started + Mac daemon stopped 2026-08-09.** **Backtest harness WRITTEN & smoke-tested: `liq_cluster_backtest.py`** — builds side-aware liquidation-cluster magnets from a trailing window (no-lookahead), tags trend_pullback trades with distance-to-nearest-DIRECTION-ALIGNED cluster (long=sell-cluster below = longs flushed / short=buy-cluster above = shorts squeezed), partitions + chrono train/test. Only tests trades inside the liq-data coverage window (so n is small until weeks accumulate; script warns + degrades gracefully). Run in ~2-4 weeks: download the `liquidations-*` Actions artifacts into a dir → `python liq_cluster_backtest.py --data-dir <dir>`. Promote to a live `mechanical_setups.py` feature ONLY if NEAR-aligned TEST net clearly beats ALL and is ≥~0; else fork C. **NEXT (data-gated, ~2-4 weeks): once enough liquidation data accumulates, build a liq-CLUSTER level map (bin liquidation notional by price into magnets), tag historical trades with distance-to-nearest-liq-cluster, and backtest (train/test) whether proximity predicts reversals. If it carries robust OOS signal → first non-price feature/signal; if empty → the market is efficient to us and fork C (analyst-tool) is the honest end state.** `liquidity_sweep_long` remains a crude price-only proxy until then.
4. **Order flow: CVD + OI-delta.** Bybit public trades + OI. CVD (aggressive buy vs sell) and OI-delta (price↑+OI↑ = real; price↑+OI↓ = short-cover fakeout) catch failing moves before entry. Cheap, high-signal.
...
- [ ] **Check head-to-head data** (recurring) — watch `logs/performance/head_to_head.md` after each `eval-scan`. It reads INSUFFICIENT until mechanical crosses ≥20 evaluated trades (est. ~early-mid Aug 2026 at the new 4h cadence). Judge on **expectancy per source**, NOT raw counts — mechanical fills ~6×/day vs Claude's 3×/day, so it's asymmetric (see "Current Status & Roadmap" flip caveat). Baseline as of 2026-07-28: mechanical 7 trades (+0.466R, needs 13 more); Claude 283 (−0.107R).
- [ ] **Per-signal T1 + direction review (data-gated, ~20 mechanical evals)** — at the SAME ≥20-trade mark as the head-to-head, use each trade's `sim_t1_0.75` vs `sim_t1_1.0` eval fields to decide, PER SIGNAL, whether to hard-set T1 to 0.75R (vs the current ≤1.0R structural). Adjust ranking/direction bias ONLY if mechanical's OWN evals (not the Claude-era 290-trade combo) show a per-direction/confluence edge. This is the honest version of the shelved "Tier 3 bias-toward-shorts" idea: confluence≥3 is ALREADY enforced for every setup (`setup_violations`), and mechanical already ranks by out-of-sample expectancy — so NO pre-emptive short bias (it would also starve the one long signal `liquidity_sweep_long`). Let the data earn any change. Raised 2026-07-29 after Tier 1+2 signal changes shipped.
- [ ] **Next lever — hunt a 5th/6th validated signal** — the real multiplier for setups/run (only 4 live; scans yield ~1/run because coins aren't the bottleneck, signal count is). Use `historical_backtester.py` (`--optimize`/`--validate`/`--walkforward`) to search candidate patterns; promote only those passing out-of-sample train/test + robustness into `_check_validated_signals` + `mechanical_setups.EXPECTANCY/SETUP_TYPE/SETUP_RULE`. Promoted 2026-07-29 (from the Jul 29 `backtest`): `liquidity_sweep_long` is now LIVE (both TFs); `macd_momentum_long` dropped (overfit) and `rsi_rejection_short` gated to 1h-only. Still HELD: `failed_breakout_short_stacked` (passed both TFs but 4h N=12/0% robust — fragile), `liquidity_sweep_short` (single-TF only). Own focused session; deferred 2026-07-28 pending more head-to-head data.
