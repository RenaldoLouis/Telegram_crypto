...
- **Shadow lane (`source="shadow"`, 2026-09-23)**: gated WATCH candidates whose unified-backtest TEST hit rate is below `SURFACE_MIN_HIT_RATE` (0.60) are persisted and evaluated but never shown and never counted (same exclusions as watch). Purpose: keep forward v2 data flowing on the 50–55% rules without polluting what the user sees. `main.split_surfaced_shadow`.
