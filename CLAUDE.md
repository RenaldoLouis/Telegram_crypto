...
7. **One trade model everywhere.** A signal is detected on CLOSED bars only (`signal_rules.detect_at`), entered at market at the next 15m open, and simulated by `trade_sim.simulate` — in the backtester AND the evaluator. Any new signal, filter, or management rule is validated in `unified_backtest.py` and nowhere else; a rule that lives only in the live path or only in a backtest is a bug.
