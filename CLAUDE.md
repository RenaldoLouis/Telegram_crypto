...
**THE SUCCESS METRIC (redefined by the user 2026-09-23) — HIT RATE of surfaced suggestions, net of cost.** A suggestion counts as *profitable* when the trade, managed as the brief says (50% off at T1, stop to breakeven, +0.3R lock after 1R, remainder to T2 or expiry), closes with **net-of-cost blended R > 0** (`profitable` field, `trade_sim.py`). **The bar: ≥70% of surfaced suggestions profitable, sustained over ≥30 forward-evaluated trades (`HIT_RATE_TARGET_PCT` / `HIT_RATE_MIN_TRADES`), WITH net expectancy > 0** — the expectancy guard exists so the hit rate cannot be bought with tiny targets and huge stops. The user will not trade real money until the eval-scan shows this. Net expectancy (the previous north-star metric, +0.05R/100 trades) stays visible in every readout as the sanity check; it is no longer the gate. History: the 2026-08-02 audit killed gross-R and win-rate-on-gross as vanity metrics — that still holds; *net* hit rate under the managed model is a different, honest number.

**CURRENT STATE (updated 2026-09-23): MEASUREMENT CHAIN REBUILT (v13.0); edge still unproven.** The 2026-09-23 whole-system audit found that six months of "no edge" numbers were produced by a chain that never simulated the same trade twice: the live detector fired on the still-OPEN 4h bar (the backtester only saw closed bars); the evaluator's "entry zone" was a market order filled at the candle CLOSE and stoppable by that candle's own pre-fill range; the backtester held trades 8 days with no costs while the evaluator gave them 2 days with costs; and the WATCH lane republished gate-rejected setups (90% of watch rows were confluence 1–2, net −0.20R) which was what the user saw most days. On top of that the EXECUTE book produced ~0.6 trades/day, so the ±0.05R bar would have needed ~1,500 trades (7 years) to resolve. **Every pre-v13 statistic is therefore non-comparable — neither proof of edge nor proof of no-edge.** v13.0 makes backtest and eval one code path (`signal_rules.py` + `trade_sim.py` + `unified_backtest.py`), gates WATCH, and restarts the forward clock (`eval_engine="v2_market_open"`, version marker v13.0). The prior conclusion "price-pattern engineering is a dead end" is **withdrawn as unestablished**; the liquidation-cluster (fork A) and CVD forks continue but must run on the unified engine before any promotion. **FORWARD TEST RUNNING (locked 2026-09-23 late, `progress.md` banner "PRE-REGISTERED FORWARD TEST"):** the one candidate, `range_reversion_short` 4h + `vol_spike≥1.5`, is frozen and paper-tracked as WATCH under the limit-open entry; decision at n=30 v2-era surfaced trades (≥70% + net>0 → EXECUTE small / 60–70% → extend to 50 / <60% → retire the 4h mean-reversion family). **Do not tune it while the clock runs; new ideas go to the shadow lane.** Honest prior: a 33-symbol holdout came back 50% / −0.21R on test and the rule's monthly profitable% decayed Apr 82% → Sep 50% on both universes, so the test is expected to fail — plan the next product (weekly cross-sectional momentum, own metric) in parallel. Same night: CI had been dropping ~half of 4h closes (cron fixed) and the Bybit fetch got a circuit breaker (`BybitFetcher._api`).
...
6. **Only surface what we would stand behind (EXECUTE vs WATCH — re-reframed 2026-09-23).** Two tiers remain: **EXECUTE** (a validated signal that cleared the structural gates → counted book) and **WATCH** (a *watch-tier signal* still building its forward sample → `source="watch"`, paper-tracked, excluded from the edge book). **As of v13.0 a WATCH candidate must pass the SAME structural gates (`WATCH_REQUIRES_GATES=True`)** — no gate-rejected execute signals, no "observation" fallback — **and only a rule with TEST hit rate ≥ `SURFACE_MIN_HIT_RATE` (0.60) resting on ≥ `SURFACE_MIN_TEST_N` (30) test trades is shown — every candidate clearing both bars is surfaced, not just the first (2026-09-23); the rest go to the `shadow` lane (logged + evaluated, never shown).** The forward test on the one candidate is PRE-REGISTERED in `progress.md` (decision rule fixed before data; the rule is frozen while the clock runs). The 2026-08-20 "never silent" rule is retired: the audit showed the always-surface lane was the gate-rejected pile (net −0.20R) and it dominated what the user saw. **An empty brief is a valid brief.** Every suggestion that IS surfaced counts toward the hit-rate metric.
7. **One trade model everywhere.** A signal is detected on CLOSED bars only (`signal_rules.detect_at`), entered per `config.ENTRY_MODEL` (default: post-only limit at the next 15m open, 30-min wait, maker fee) and simulated by `trade_sim` — in the backtester AND the evaluator. Any new signal, filter, or management rule is validated in `unified_backtest.py` and nowhere else; a rule that lives only in the live path or only in a backtest is a bug.
...
main.py  (orchestrator, async)
  │
  ├── fetchers/bybit_data.py       → BybitFetcher (2026-09-23: every call via _api() — circuit breaker:
  │                                    network failure → flip host bybit↔bytick, 4 consecutive → BybitHostsDown
  │                                    → main.py retries the snapshot 3×; API 403 → host dead for the run):
  ...
  │                                    v13.0 freshness gate (SIGNAL_MAX_AGE_MIN) drops stale closed-bar signals
  ├── signal_rules.py              → v13.0: THE signal rules (compute_indicators + detect_at(df,i,tf)) shared by
  │                                    the live scan (last CLOSED bar) and unified_backtest.py (every closed bar)
  ├── trade_sim.py                 → v13.0: THE trade simulator (market@next-open entry, wick stops, T1 partial +
  │                                    BE + trail, costs, `profitable`) shared by weekly_eval + unified_backtest
  ...
  ├── liq_fade_backtest.py         → Research item 1 (2026-09-23): liquidation forced-flow fade on collector data → no edge
...
Runs on the main CI schedule (`7 */2` UTC; skipped on the +37/+65 min scan-only retry runs). Zero Claude tokens, single Bybit ticker call per run (tries `api.bytick.com` direct, then `api.bybit.com` via VPN — US-located runners get a 403 from bytick).
...
# UNIFIED backtester (v13.0 — the ONLY validation path for anything that ships; zero tokens)
...
python unified_backtest.py --entry market                     # compare against the legacy taker fill (default = config.ENTRY_MODEL)
...
# LEGACY historical strategy backtester (pre-v13 trade model: 8-day hold, no costs, closed-bar signals but a
# different exit model — its expectancies are NOT comparable to live and must not be used to promote signals)
...
- **Momentum pulse + mechanical scan**: GitHub Actions (`.github/workflows/momentum_pulse.yml`). **Schedule fixed 2026-09-23:** the old `0 */2` cron was delayed/dropped by GitHub (~6 runs/day at random offsets → ~half of 4h closes missed inside the 90-min freshness window). Now `7 */2` (main pulse+scan, off the top of the hour) plus scan-only RETRIES at `37 0,4,8,…` and `5 1,5,9,…` (+37/+65 min after each 4h close; pulse step skipped; `SCAN_QUIET_IF_EMPTY=1` → no archive/Telegram unless something is surfaced; dedup drops repeats). Check `gh run list --workflow=momentum_pulse.yml` monthly — if closes are still being missed, the fix is more retry crons, not a longer freshness window. One job runs the pulse, then `main.py`, then a single race-safe commit. The scan runs **mechanical-only** (as of 2026-08-26 this is true everywhere — `PRIMARY_SOURCE="mechanical"` skips the Claude call outright; CI also has **no `ANTHROPIC_API_KEY`** as a second layer). Reuses the WireGuard VPN (`WG_CONF`) for Bybit access. Auto-commits `hot_list.json` + `logs/setups/` + `logs/briefs/` back to the repo. Secrets are GitHub repository secrets.
- **Local scan**: macOS `launchd` (`~/Library/LaunchAgents/com.user.cryptoscreener.plist`) → runs `run_scan.sh` (the alias-equivalent wrapper) **5 min after every 4h bar close: 07:05 / 11:05 / 15:05 / 19:05 / 23:05 / 03:05 local (UTC+7)** (moved 2026-09-23 from 09/17/22 — a backup for the flaky CI schedule; a run while the Mac was asleep fires on wake and simply skips stale signals). As of 2026-08-26 this **no longer calls Claude** either (`PRIMARY_SOURCE="mechanical"` gates the scan Claude call off even though the local `.env` still has the key) → identical mechanical output to CI, zero scan tokens. Claude spend is ZERO everywhere as of 2026-09-10 (delta analysis retired). **Requires VPN up at those times** (Bybit + Telegram are ISP-blocked); a scan with VPN down fails at data fetch. launchd logs go to `~/Library/Logs/cryptoscreener_scan.{log,err}` (outside the repo so `git add -A` won't commit them). Reload after editing the plist: `launchctl unload … && launchctl load -w …`.
...
- **CI VPN is SPLIT-TUNNEL as of 2026-09-17** (`.github/actions/bybit-vpn`, shared by all three workflows): only Bybit's CloudFront IPs (resolved on the clean runner net, pinned in `/etc/hosts`) route through WireGuard; GitHub/pip/Telegram go direct (Telegram is NOT blocked for US runner IPs — the ISP block is local-only). This ended the Sep 15–17 outage mode where a degraded VPN endpoint took the runner's own GitHub connection down → uniform ~8-min "operation was canceled" runs. The action health-gates on Bybit answering through the tunnel (5 rebuild retries, then fast-fail); both collectors additionally RETRY across their 5.5h window (relaunch with remaining budget after a WS death) instead of forfeiting it. `collector_health.yml` (daily 06:45 UTC, no VPN) Telegram-alerts when any workflow drops under 50% success in 24h — collector outages are now visible within a day. If the alert keeps firing, the WG endpoint itself is bad → rotate the `WG_CONF` secret to a fresh server config.
...
- **Closed bar / open bar (v13.0)**: Bybit's newest kline row is the bar in progress. `BybitFetcher._split_closed` drops it; indicators and signals use closed bars only, the open bar only supplies `current_price`. Pre-v13 the detector ran on the open bar → signals could fire then un-fire, and the live population differed from the backtested one (audit 2026-09-23).
- **Signal freshness (`SIGNAL_MAX_AGE_MIN`, v13.0)**: a closed-bar signal is actionable only within 90 min (4h) / 45 min (1h) of its bar close, because the validated trade enters at the NEXT bar's open. CI scans at every even UTC hour, so each 4h close is caught by the next run; the runs in between skip 4h signals as stale (logged).
- **`profitable` (v13.0 hit-rate metric)**: `net_blended_rr > 0` — the managed trade (50% at T1, BE stop, +0.3R lock after 1R, remainder to T2/expiry) closed green AFTER fees/slippage/funding. The user's bar is ≥70% of surfaced suggestions profitable over ≥30 forward trades with net expectancy > 0. Distinct from `won` (gross actual_rr > 0, legacy).
- **Shadow lane (`source="shadow"`, 2026-09-23)**: gated WATCH candidates whose unified-backtest TEST hit rate is below `SURFACE_MIN_HIT_RATE` (0.60), or rests on fewer than `SURFACE_MIN_TEST_N` (30) test trades, are persisted and evaluated but never shown and never counted (same exclusions as watch). Purpose: keep forward v2 data flowing on the 50–55% rules without polluting what the user sees. `main.split_surfaced_shadow`.
- **Entry model (`ENTRY_MODEL`, 2026-09-23)**: `"limit_open"` = post-only limit at the next 15m open, filled only if price trades through it within `LIMIT_WAIT_BARS` (2 = 30 min); maker fee on entry and take-profits, taker on stops. Fills 96–99% and adds +0.03..+0.08R per trade vs market on every rule (pure fee effect). Used by evaluator, harness and brief alike; unfilled setups get status `not_filled` (not a loss, not counted). `"market"` = legacy taker fill. Implemented in `trade_sim.simulate_limit` / `cost_rr_legs`.
- **`eval_engine`**: stamped on every eval record; `"v2_market_open"` = scored by `trade_sim` with market-at-next-open entry (post-2026-09-23). Records without it were scored by the retired zone-trigger evaluator and are not comparable.
...
- **Validated signals (v13.0 status, 2026-09-23)**: the signal rules live in `signal_rules.py` (shared by scan + harness). **Under the unified trade model NOTHING currently passes the SHIP bar** (≥70% profitable net, net exp > 0, n≥30 on TEST), so **every rule runs as WATCH** (gated, paper-tracked, forward-scored by eval engine v2) and the EXECUTE lane is empty until a rule clears the bar on forward data (`head_to_head.md` promotion table). Unified-backtest TEST hit rates (30 symbols, 180 d, last-40% chrono): `range_reversion_short` 4h with the `vol_spike≥1.5` gate 65.9% (n=41; all-period 75% / +0.19R — the only candidate), `range_reversion_long` 4h 73% (n=11, too thin), `failed_breakout_short` 4h 54.5%, `trend_pullback_short` 4h 52.6%, `liquidity_sweep_long` 1h 51% (4h variant DISABLED, 44%), `rsi_bounce_long` 4h 51%, `rsi_rejection_short` DISABLED (39%). The management sweep (tighter T1 / wider stop) raised hit rate a few points but never lifted net expectancy above zero — hit rate and payoff diverge; the guard exists for that reason. Filter study: 188 cuts, ~9 expected false positives, one real lead (volume gate above). The old validation numbers quoted below this line (e.g. "+0.28 expect", "★ STRONG") came from the legacy backtester's different trade model and are historical only.
